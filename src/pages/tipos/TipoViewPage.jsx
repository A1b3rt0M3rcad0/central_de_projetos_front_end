...
export default function TipoViewPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const tipoData = location.state?.item;
  ...
  useEffect(() => {
    if (!tipoData?.id) {
      Swal.fire("Erro", "Tipo não fornecido", "error");
      navigate(-1);
      return;
    }
    ...
    const fetchProjects = async () => {
      try {
        const response = await tipoAPI.getProjectsByTipo(tipoData.id);
        setProjects(response.data.content || []);
      } catch (error) {
        console.error("Erro ao carregar projetos:", error);
        // Se não houver projetos, não mostrar erro
        setProjects([]);
      } finally {
        setLoading(false);
      }
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600 mt-1">
                    {projects.length}{" "}
                    {projects.length === 1 ? "projeto" : "projetos"} deste tipo
                  </p>
                </div>
              </div>
            </div>
              ...
              {projects.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-lg">
                    Nenhum projeto encontrado com este tipo
                  </p>
                </div>
              ) : (
