...
const userApi = {
  ...
  getUsersWithPagination: (pageSize, page) =>
    api.get(`/users/pagination/${pageSize}/${page}`),
  getUsersWithPaginationAndFilter: (pageSize, page, search) => {
    const params = new URLSearchParams();
    if (search) params.append("search", search);
    return api.get(
      `/users/pagination-filter/${pageSize}/${page}?${params.toString()}`
    );
  },
  ...
  getAllUserByRole: (role) => api.get(`/users/all/${role}`),
  ...
  validateEmailExists: (email) =>
    api
      ...
      .then(() => true)
      .catch(() => false),
  getUsersByRole: (role) => api.get(`/users/all/${role}`),
