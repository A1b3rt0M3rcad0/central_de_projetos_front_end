...
const empresaAPI = {
  getAllEmpresas: () => api.get("/empresa/all"),
  getEmpresasWithPagination: (pageSize, page) =>
    api.get(`/empresa/pagination/${pageSize}/${page}`),
  getEmpresasWithPaginationAndFilter: (pageSize, page, search) => {
    const params = new URLSearchParams();
    if (search) params.append("search", search);
    return api.get(
      `/empresa/pagination-filter/${pageSize}/${page}?${params.toString()}`
    );
  },
  ...
  getCountEmpresas: () => api.get("/empresa/count/all"),
